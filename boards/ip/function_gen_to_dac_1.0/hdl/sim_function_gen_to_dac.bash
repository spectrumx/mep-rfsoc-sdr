...
usage() {
  cat <<'EOF'
Usage: ./sim_function_gen_to_dac.bash [--gui] [-h|--help]
...
xelab -debug typical function_gen_to_dac_tb -s sim_function_gen_to_dac

if [[ "${USE_GUI}" -eq 1 ]]; then
  xsim sim_function_gen_to_dac -gui
else
  xsim sim_function_gen_to_dac -runall
fi
