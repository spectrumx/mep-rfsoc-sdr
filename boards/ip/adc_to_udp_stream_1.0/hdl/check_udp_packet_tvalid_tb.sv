...
//  Inserts periodic S01 TVALID gaps while checking UDP packet payloads
//  against the accepted input stream.
//
///////////////////////////////////////////////////////////////////////////////

`timescale 1 ns / 1 ps

module check_udp_packet_tvalid_tb;
    ...
    localparam int S01_TVALID_PAUSE_PERIOD = 257;
    ...
    int s01_tvalid_pause_events;
    int s01_tvalid_pause_cycles;
    int payload_start_delta;
    ...
    task verify_packet;
        ...
        // Payload bytes must match a contiguous span of the accepted S01 sample stream.
        first_payload_word = {pkt_byte_stream[107], pkt_byte_stream[106]};
        payload_start_index = -1;
        for (i = 0; i < input_sample_count; i = i + 1) begin
            if ((payload_start_index < 0) && (input_samples_queue[i] == first_payload_word)) begin
                payload_start_index = i;
            end
        end
        for (i = 0; i < 8192; i = i + 1) begin
            expected_sample_index = payload_start_index + (i / 2);
            if ((payload_start_index >= 0) && (expected_sample_index < input_sample_count)) begin
                exp_word = input_samples_queue[expected_sample_index];
            end else begin
                exp_word = 16'h0;
            end
        ...
        $display("[CHK] ========================================");
            ...
            if (payload_start_index < 0) begin
                ...
                $display("[CHK] payload first word 0x%04x was not found in accepted S01 samples",
                    first_payload_word);
            end else begin
                current_payload_delta = payload_start_index - (pkt_num * WORDS_PER_PACKET);
                $display("[CHK] payload_start_index = %0d start_delta = %0d first_word = 0x%04x",
                    payload_start_index, current_payload_delta, first_payload_word);
                if (payload_start_delta < 0) begin
                    ...
                    fail_count = fail_count + 1;
                    $display("[CHK] payload start delta changed: expected=%0d actual=%0d",
                        payload_start_delta, current_payload_delta);
                end
            end
    ...
    initial begin
        ...
        s01_tvalid_pause_events = 0;
        s01_tvalid_pause_cycles = 0;
        ...
        for (i = 0; i < NUM_INPUT_BEATS; i = i + 1) begin
            if ((i > 0) && ((i % S01_TVALID_PAUSE_PERIOD) == 0)) begin
                pause_cycles = 1 + ((i / S01_TVALID_PAUSE_PERIOD) % 3);
                s01_axis_tvalid = 1'b0;
                s01_tvalid_pause_events = s01_tvalid_pause_events + 1;
                s01_tvalid_pause_cycles = s01_tvalid_pause_cycles + pause_cycles;
                $display("[CHK] S01 TVALID pause after input_beats=%0d cycles=%0d time=%0t",
                    i, pause_cycles, $time);
                for (gap = 0; gap < pause_cycles; gap = gap + 1) begin
        ...
        $display("S01 TVALID pauses: events=%0d cycles=%0d",
            s01_tvalid_pause_events, s01_tvalid_pause_cycles);
        if (s01_tvalid_pause_events == 0) begin
            $display("FAIL: no S01 TVALID pauses were inserted");
        end
        $finish;
    end
