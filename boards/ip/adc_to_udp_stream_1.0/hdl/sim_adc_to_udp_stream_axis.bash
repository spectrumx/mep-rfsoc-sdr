...
usage() {
  cat <<'EOF'
Usage: ./sim_adc_to_udp_stream_axis.bash [--gui] [-h|--help]

Options:
  --gui       Launch xsim in GUI mode and load adc_to_udp_stream_axis_tb_sim.wcfg if present
...
WCFG_FILE="./adc_to_udp_stream_axis_tb_sim.wcfg"
...
xelab -debug typical adc_to_udp_stream_axis_tb glbl -s sim_adc_to_udp_stream_axis

if [[ "${USE_GUI}" -eq 1 ]]; then
  if [[ -f "${WCFG_FILE}" ]]; then
    xsim sim_adc_to_udp_stream_axis -gui -view "${WCFG_FILE}"
  else
    xsim sim_adc_to_udp_stream_axis -gui
  fi
else
  xsim sim_adc_to_udp_stream_axis -runall
fi
