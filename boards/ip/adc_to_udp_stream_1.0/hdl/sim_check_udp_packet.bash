...
usage() {
  cat <<'EOF'
Usage: ./sim_check_udp_packet.bash [--gui] [-h|--help]

Options:
  --gui       Launch xsim in GUI mode and load check_udp_packet_tb_sim.wcfg if present
...
WCFG_FILE="./check_udp_packet_tb_sim.wcfg"
...
xelab -debug typical check_udp_packet_tb glbl -s sim_check_udp_packet

if [[ "${USE_GUI}" -eq 1 ]]; then
  if [[ -f "${WCFG_FILE}" ]]; then
    xsim sim_check_udp_packet -gui -view "${WCFG_FILE}"
  else
    xsim sim_check_udp_packet -gui
  fi
else
  xsim sim_check_udp_packet -runall
fi
